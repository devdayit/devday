...
const RandomVideo = observer(
  class RandomVideo extends Component {
    ...
    random(list, randomIndex, gitHubDataService) {
      ...
      key &&
        gitHubDataService.read("pastEvents", key).then(event => {
              ...
              <Step.Group fluid>
                <Step active style={{ width: "50%" }}>
                  ...
                  <Step.Content>
                    <Step.Title>
                      <Link
                        to={`/past-event/${key.substring(0, key.length - 5)}`}
                      >
                    ...
                    <Step.Description>
                      ...
                      e rivivi questo talk!
                    </Step.Description>
                  </Step.Content>
                </Step>
                <div
                  className={
                    this.videoVisible ? "normal-video" : "static-video"
                  }
                >
                  <Step>
                    <Step.Content style={{ width: "100%" }}>
                      <div className="videowrapper">
                        <iframe
                          ...
                          height={315}
                          title={event.name}
                          src={`https://www.youtube.com/embed/${event.youtube}?autoplay=1`}
                          frameBorder={0}
                          allowFullScreen
                        />
                      </div>
